...
```{r set up samplesheet for display, echo=FALSE, warning=FALSE}
no_reads <- c()
mean_rl = c()
no_mapped_reads = c()
sample_ids =c()
for (sample_id in factor(samplesheet$sample_id)) {
    ...
    mr <- sum(abundance_table_display[,sample_id])
    no_mapped_reads <- append(no_mapped_reads, round(mr,0))
    sample_ids <- append(sample_ids, sample_id)

}

tmp_df <- data.frame(no_reads, mean_rl,  
...
samplesheet <- samplesheet %>%
  mutate(perc_mapped = round((no_mapped_reads/no_reads)*100,2))
